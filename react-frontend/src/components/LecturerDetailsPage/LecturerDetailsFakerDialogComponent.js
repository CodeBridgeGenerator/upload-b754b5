...
import lecturerDetailsFakerFactory from "./lecturerDetailsFakerFactory";
...
const LecturerDetailsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = lecturerDetailsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("lecturerDetails").create(elem));
...
export default connect(mapState, mapDispatch)(LecturerDetailsFakerDialogComponent);
