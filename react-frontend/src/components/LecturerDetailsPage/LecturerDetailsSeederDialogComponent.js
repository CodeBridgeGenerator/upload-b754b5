...
import seederData from "./lecturerDetails.seeder.json";
...
const LecturerDetailsSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("lecturerDetails").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(LecturerDetailsSeederDialogComponent);
